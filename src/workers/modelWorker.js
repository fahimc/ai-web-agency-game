...
self.onmessage = async (event) => {
  ...
  try {
    const { employee, task, context, settings, state, complex, apiKey } = payload;
    const model = complex ? settings.complexModel : settings.fastModel;
    if (!apiKey && !settings.proxyUrl) throw new Error('OpenAI API key or proxy URL missing. Open Menu > Settings, add your key, then press Resume work.');
    const system = [
    ...
    if (settings.proxyUrl) {
      ...
      requestPayload.apiKey = apiKey || undefined;
      requestPayload.openaiPayload = { model, input: requestPayload.input };
    } else {
