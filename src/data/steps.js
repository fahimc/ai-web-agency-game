export const previewSteps = [
  ...
  },
  {
    ...
    task: 'Build the final production customer website as one complete single-file HTML document using the selected design direction, colour palette, approved pages, approved sections, and MicroAgency Blocks component system. Requirements: return only valid HTML starting with <!doctype html>; include all CSS inside <style>; no external libraries; include a visible navigation menu where every link uses a same-page hash such as href="#services"; create a matching real section id for every nav link such as id="services"; create a real section for every approved page even if sensible dummy production copy is needed; do not include agency-facing labels such as example, sample, concept, preview, customer website, visual direction, or design direction inside the customer site; responsive mobile-first layout; strong conversion copy; clear CTAs; accessible semantic markup; polished modern design. Use local /placeholders assets where imagery is useful. A small inline script is allowed only to make hash navigation scroll reliably.',
  },
];
